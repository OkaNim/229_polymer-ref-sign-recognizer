...
def extract_refsign(src_001, TOK, TAG, PTAG=None):
    REFSIGN = []
        ...
        if "S" in x : j2 = i
            ...
            for j in range(i + 1, len(TAG)):
                if "E" in TAG[j]:
                    ...
                    break
        if j2 != -1:
            refsign = " ".join(TOK[i : j2 + 1])
            REFSIGN.append([refsign, "_", "_"])

            if PTAG != None:
                POLYMER = []
                for j in range(i, j2 + 1):
                    k2 = -1
                    if PTAG[j][0] == "S": k2 = j
                    elif PTAG[j][0] == "B":
                        k2 = src_001.iter_end(PTAG, start=j, end_word="E")                         # import
                    if k2 != -1:
                        polymer = " ".join(TOK[j : k2 + 1])
                        POLYMER.append(polymer)
                        if k2 < len(TOK) - 1 and TOK[k2 + 1] == ")": k2 += 1
                        attached = " ".join(TOK[k2 + 1 : j2 + 1])
                if POLYMER != []:
                    REFSIGN[-1][1] = "; ".join(POLYMER)
                    REFSIGN[-1][2] = attached
...
def modify_polymer_tag(TOK, TAG, REFSIGN_ABB):
    for i, tag in enumerate(TAG):
        if tag[0] == "S":
            for abb in REFSIGN_ABB:
                if abb in TOK[i]:
                    TAG[i] = "O"
                    break
        if tag[0] == "E":
